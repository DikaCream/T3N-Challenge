...
- `getMemberDelegation` / `addMemberDelegationGrants` for scoped delegation with `allowed_hosts`; this is also the working path to authorise egress for an individual tenant
...
| 9 | **`onboard` (no `--live`)** | the request bodies with `{{profile.*}}` markers **unresolved**. That is the privacy evidence |
