...
- `getMemberDelegation` / `addMemberDelegationGrants` — scoped delegation with `allowed_hosts`
...
`RPC Error: Internal error` — no field, no component, no hint. The raw JSON-RPC
...
cannot be read back** — not from `list`, `listDetailed`, `maps`, or the ledger —
even though map ACLs are keyed on it, which meant a second `init` could resolve those
